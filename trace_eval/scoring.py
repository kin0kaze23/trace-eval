...
REQUIRED_JUDGES = {"reliability", "retrieval", "tool_discipline"}
...
def compute_scorecard(
    ...
    total_weight = sum(profile.get(n, 0) for n in scorable)
    total_score = 0.0
    for name in scorable:
        weight = profile.get(name, 0) / total_weight if total_weight > 0 else 0
